...
export DEVICE_NUM=2
export RANK_SIZE=4
export HOSTFILE=~/mindspore/model_zoo/official/cv/resnet/scripts/hostfile
...
if [ $# == 3 ]
then	    
        /usr/local/openmpi-4.0.3/bin/mpirun --allow-run-as-root -n $RANK_SIZE --hostfile $HOSTFILE -x NCCL_DEBUG=INFO -x GLOG_v=1 -x NCCL_SOCKET_IFNAME=ens11f0 --mca btl_tcp_if_include ens11f0 --output-filename log_output \
	      python train.py --net=$1 --dataset=$2 --run_distribute=True \
	      --device_num=$DEVICE_NUM --device_target="GPU" --dataset_path=$PATH1 &> log &
        echo $profile
        if [[ $profile == "1" ]]
        then
                while [ "`top -n 1 -b | grep -E "testADSLab"`" == '' ]
                do
                    sleep 1
                done
                ./test.sh &> testADSLab.log &
        fi
fi
